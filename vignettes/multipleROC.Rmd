...
```
You can calculate all the ratio by hand. But, there are the results in the `x` object previously we maid. 
...
longdf <- x$df %>% pivot_longer(cols=sens:spec,names_to = "rate")
...
```{r}
x$df[which.max(x$df$sum),]
```
You can see when the x value is 0.5450, the sum of sensitivity and specificity is the highest. So the best cutoff value of x is 0.5450. This value can convert to optimal value of height by logistic regression model(x$fit) equation.
...
So you can calculate the optimal cutpoint of height as follows
...
The AUC is calcuated by the simpleAUC function
```{r}
simpleAUC(x$df)
x$auc
```
